...
<div class="card-block table-border-style">
	<div class="table-responsive">
		...
		<?php 
			require_once ('../PHP/conexao.php');
			$sql = "SELECT Name, Cores_Threads, Clock, Socket, Litho, Cache, TDP, Realeased FROM dc463jjk_benchking.cpu";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
            	// output data of each row
            	while($row = $result->fetch_assoc()) {
					echo "<tr><td>" . $row["Name"]. "</td><td>" . $row["Cores_Threads"]."</td><td>". $row["Clock"] . "</td><td>". $row["Socket"]. "</td><td>". $row["Litho"]. "</td><td>". $row["Cache"]. "</td><td>". $row["TDP"]. "</td><td>". $row["Realeased"];
